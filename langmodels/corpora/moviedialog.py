...
class MovieDialogCollection(Corpus):
    ...
    def __iter__(self):
        ...
        with cursor:
            for record in cursor:
                try:
                    if record['text'] is not None and type(record['text']) == type('a'):
                        yield record['id'], record['text'], record['V'], record['A'], record['D']
                    else:
                        pass                        
                except KeyError:
    ...
    def get_tokens(self) -> list:
        return [(id, self.tokenize(text)) for id, text, V, A, D in self]

    def get_skip_tokens(self, n=2, s=2):
        return [(id, Corpus.skip(self.tokenize(text), n=n, s=s)) for id, text, V, A, D in self]

    def get_text(self) -> list:
        return [(id, text) for id, text, V, A, D in self]
    
    def get_labels(self) -> list:
        return [(id, V, A, D) for id, text, V, A, D in self]
